...
Fixed Fixed::operator+(const Fixed& object)
{
	//std::cout << "member operator overloads + called" << std::endl;
...
//member operator overloads ++ (Post-increment overloading)
Fixed Fixed::operator++(int)
{
	//std::cout << "member operator overloads++ (Post-increment overloading) called" << std::endl;
	float temp = this->toFloat();
	this->setRawBits(this->getRawBits() + 1);
	return (Fixed(temp));
}

//member operator overloads ++ (Pre-increment overloading)
Fixed Fixed::operator++(void)
{
	//std::cout << "member operator overloads++ (Pre-increment overloading) called" << std::endl;
	this->setRawBits(this->getRawBits() + 1);
